...
if not os.path.exists(COMMENT_IMAGES_FOLDER):
    ...
    logger.info(f"Created folder: {COMMENT_IMAGES_FOLDER}")
...
def submit_comment():
    ...
    logger.info(f"Submitting comment from user: {employee_id} for page: {page_url}")

    # Return an error if comment or page_url are missing
    if not comment or not page_url:
        logger.warning("Comment or page URL missing")
    ...
    if employee_id:
        try:
            ...
            logger.info(f"User {employee_id} found with ID: {user_id}")
    ...
    if base64_image:
        try:
            ...
            with open(screenshot_full_path, 'wb') as f:
                f.write(image_data)
            logger.info(f"Screenshot saved at {screenshot_full_path}")
    ...
    try:
        ...
        logger.error(f"Error while saving comment: {e}")
        session_db.rollback()
