...
# Load the AI model
try:
    if current_ai_model != "NoAIModel":
        ai_model = load_ai_model(current_ai_model)
        logger.debug(f"Loaded AI model: {current_ai_model}")
    else:
        ai_model = None
        logger.info("AI model is disabled.")
...
try:
    if current_embedding_model != "NoEmbeddingModel":
        ...
        logger.debug(f"Loaded embedding model: {current_embedding_model}")
    else:
        embedding_model = None
        logger.info("Embedding model is disabled.")
except Exception as e:
    ...
    try:
        ...
        if keyword:
            ...
            if relevant_doc:
                ...
                logger.debug(f"Generated prompt for AI model: {prompt}")

                if ai_model:
                    answer = ai_model.get_response(prompt)
                    logger.debug(f"AI model response: {answer}")
                else:
