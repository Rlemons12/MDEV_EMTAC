...
from emtacdb_fts import (
    ...
    ImageProblemAssociation, create_position, ImageSolutionAssociation, Document, CompleteDocument, Part, Drawing, PartProblemAssociation, PartSolutionAssociation, DrawingProblemAssociation, DrawingSolutionAssociation, ProblemPositionAssociation
)
...
def update_problem_solution():
    try:
        ...
        try:
            ...
            if tsg_selected_drawing_search:
                ...
                if drawing:
                    logger.info(f"Associating drawing {drawing.id} with problem {problem.id} and solution {solution.id}")
    ...
    return redirect(url_for('troubleshooting_guide.html'))
