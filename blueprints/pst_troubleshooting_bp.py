...
def get_site_locations():
    ...
    if not all([model_id, asset_number_id, location_id]):
        return jsonify({"error": "model_id, asset_number_id, and location_id parameters are required"}), 400
    ...
    try:
        ...
        data = [{'id': loc_id, 'title': title, 'room_number': room_number} for loc_id, title, room_number in
                site_location_set]
        return jsonify(data), 200
    except Exception as e:
