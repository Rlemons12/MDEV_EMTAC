...
def create_position(area_id, equipment_group_id, model_id, asset_number_id, location_id, site_location_id,session):
    session = db_config.MainSession()()  # Obtain the session object from scoped_session
    try:
...
def add_powerpoint_to_db(session: db_config.MainSession(), title, ppt_file_path, pdf_file_path, complete_document_id, description=""):
    try:
...
def serve_image(session: db_config.MainSession(), image_id):
...
def search_documents_db(session: db_config.MainSession(), title='', area='', equipment_group='', model='', asset_number='', location=''):
...
def find_most_relevant_document(question, session: db_config.MainSession()):
    if CURRENT_EMBEDDING_MODEL == "NoEmbeddingModel":
...
def load_keywords_and_patterns(session: db_config.MainSession()):
