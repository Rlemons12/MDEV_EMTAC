...
def create_base_directories():
    ...
    info_id("🏗️ Creating essential base directories...")
...
def early_database_creation():
    ...
    try:
        ...
        if not DATABASE_URL.startswith('postgresql'):
            info_id("🗃️ SQLite detected - skipping early database creation")
            return True
        ...
        if not match:
            warning_id("⚠️ Could not parse PostgreSQL DATABASE_URL.")
            return False

        username, password, host, port, default_database_name = match.groups()

        # Ask user for database name choice
        print(f"\n🗃️ Early Database Creation")
        print(f"   📋 Default database name from config: {default_database_name}")
        ...
        try:
            ...
            try:
                ...
                if db_exists:
                    ...
                    create_db = input(f"🏗️ Create PostgreSQL database '{database_name}'? (y/n): ").strip().lower()

                    if create_db in ['y', 'yes']:
                        info_id(f"🏗️ Creating PostgreSQL database '{database_name}'...")
                ...
                if custom_name:
                    ...
                    info_id(f"⚙️ Updated DATABASE_URL for custom database name")
        ...
        warning_id(f"⚠️ psycopg2 not available: {e}")
        return False
    except Exception as e:
...
def import_modules_after_directory_setup():
    ...
    try:
        ...
        try:
            ...
            warning_id(f"⚠️ Warning: Could not import revision control module: {e}")
...
def setup_virtual_environment_and_install_requirements():
    ...
    if create_venv == 'y' or create_venv == 'yes':
        ...
        if os.path.exists(venv_dir):
            ...
            if overwrite == 'y' or overwrite == 'yes':
                info_id(f"🗑️ Removing existing virtual environment at {venv_dir}")
                shutil.rmtree(venv_dir)
            else:
                info_id("✔️ Using existing virtual environment.")

        # Create virtual environment if it doesn't exist or was removed
        if not os.path.exists(venv_dir):
            info_id(f"🏗️ Creating virtual environment at {venv_dir}...")
            try:
        ...
        try:
            info_id("⬆️ Upgrading pip in virtual environment...")
        ...
        info_id("⏭️ Skipping virtual environment creation.")
    ...
    if os.path.isfile(requirements_file):
        ...
        warning_id(f"⚠️ No requirements.txt file found at {requirements_file}. Skipping dependency installation.")
    ...
    for directory in directories_to_check:
        try:
            ...
            if not (resolved_dir.startswith(project_root) or
                ...
                warning_id(f"⚠️ Skipping directory outside project scope: {directory} -> {resolved_dir}")
                continue
            ...
            if not os.path.exists(resolved_dir):
                ...
                info_id(f"✔️ Directory already exists: {resolved_dir}")
            ...
            if db_type == 'PostgreSQL':
                ...
                try:
                    with db_config.main_session() as session:
                        ...
                        if not can_create:
                            warning_id("⚠️ Warning: User may not have CREATE privileges")
                            return False
                except Exception as e:
                    warning_id(f"⚠️ Could not check permissions: {e}")
...
def check_and_create_database_schema():
    ...
    info_id("🗄️ Setting up database schema and tables...")
    ...
    try:
        ...
        info_id("🏗️ Setting up main database tables...")
        ...
        if not main_tables:
            ...
            info_id(f"✔️ Main database ready with {len(main_tables)} existing tables")
        ...
        if ModelsConfig:
            ...
            try:
                ...
                if 'models_config' not in main_inspector.get_table_names():
                    ...
                    info_id("⚙️ Initializing AI models with default configuration...")
                    ModelsConfig.initialize_models_config_table()
                    info_id("✅ AI models configuration initialized")
                else:
                    info_id("✔️ AI models configuration table exists")

                    session = db_config.get_main_session()
                    try:
                        config_count = session.query(ModelsConfig).count()
                        if config_count == 0:
                            info_id("⚙️ Initializing empty AI models configuration...")
                            ModelsConfig.initialize_models_config_table()
                            info_id("✅ AI models configuration initialized")
                        else:
        ...
        if not revision_tables:
            ...
            info_id(f"✔️ Revision control database ready with {len(revision_tables)} existing tables")
...
def setup_audit_system():
    """Set up the database audit system - Windows Unicode Fixed Version"""
    try:
        info_id("Setting up database audit system...")

        print("\n" + "=" * 80)
        print("DATABASE AUDIT SYSTEM SETUP")
        print("=" * 80)
        print("The audit system provides comprehensive change tracking:")
        print("")
        print("CAPABILITIES:")
        ...
        print("   • SQLAlchemy Event-Based Auditing (Python integration)")
        print("   • PostgreSQL Trigger-Based Auditing (Database-level)")
        print("   • Central audit_log table + individual audit tables")
        print("   • Performance indexes for fast queries")
        print("")
        print("BENEFITS:")
        print("   • Compliance and regulatory requirements")
        print("   • Security monitoring and forensics")
        print("   • Data change history and recovery")
        ...
        if not os.path.exists(audit_setup_script):
            # Create the audit setup script from our template (NO EMOJIS VERSION)
...
def suggest_database_backup():
    ...
    try:
        ...
        if db_config.is_postgresql:
            print("🐘 For PostgreSQL, you can create a backup using:")
            print("   pg_dump -h hostname -U username -d database_name > backup.sql")
            print("")
            ...
            print("🗃️ For SQLite, you can simply copy the database file:")
            try:
                from modules.configuration.config import DATABASE_URL
                ...
                print("   cp your_database.db your_database.db.backup")
        ...
        if backup_now in ['y', 'yes']:
            ...
            info_id("⏭️ User chose to skip backup")

    except Exception as e:
        warning_id(f"⚠️ Error in backup suggestion: {e}")
...
def check_existing_data():
    ...
    try:
        ...
        warning_id(f"⚠️ Could not check existing data: {e}")
        return False, {}
...
def run_setup_scripts():
    ...
    if data_exists:
        ...
        print("")
        print("🔄 Running import scripts may create duplicates!")
        print("💡 Consider backing up your database first.")
        print("")

        proceed = input("⚠️  Continue with data import scripts anyway? (y/n): ").strip().lower()
        if proceed not in ['y', 'yes']:
            info_id("⏭️ Skipped setup scripts due to existing data.")
            return
    ...
    for script in scripts_to_run:
        ...
        if setup_state.get('audit_system_enabled'):
            print("🔍 Note: All changes will be automatically audited")

        user_input = input(f"▶️ Run {script}? (y/n): ").strip().lower()
        ...
        if user_input == 'y' or user_input == 'yes':
            ...
            try:
                ...
                continue_input = input("⏭️ Continue with the next script despite the error? (y/n): ").strip().lower()
                ...
                continue_input = input(
                    "⏭️ Continue with the next script despite the missing file? (y/n): ").strip().lower()
                if continue_input != 'y' and continue_input != 'yes':
                    info_id("🛑 Setup aborted by user after missing script file.")
                    sys.exit(1)
        else:
            print(f"⏭️ Skipping {script}...")
            info_id(f"⏭️ User chose to skip {script}")
...
def run_post_setup_associations():
    ...
    try:
        ...
        info_id("🖼️ Associating parts with images...")
        if db_maintenance:
            db_maintenance.associate_all_parts_with_images(export_report=True)
        else:
            warning_id("⚠️ Database maintenance module not available, skipping associations")
...
def display_setup_summary():
    ...
    print("\n" + "=" * 70)
    print("🚀 EMTAC Enhanced Database & Application Setup")
    print("=" * 70)
    print("This comprehensive setup script will configure your EMTAC system including:")
    print("")
    print("🗄️  DATABASE & SCHEMA:")
    print("   • Database creation BEFORE module imports (fixes import errors)")
    print("   • Database tables and relationships")
    print("   • Performance indexes and optimizations")
    print("   • PostgreSQL extensions (if using PostgreSQL)")
    print("   • Full-text search capabilities")
    print("")
    print("🔍 AUDIT & COMPLIANCE:")
    print("   • Comprehensive database change tracking")
    print("   • User activity monitoring")
    print("   • Before/after data snapshots")
    print("   • Regulatory compliance support")
    print("")
    print("🤖 AI & INTELLIGENCE:")
    print("   • AI model configurations")
    print("   • Embedding model setup")
    print("   • Smart search capabilities")
    print("")
    print("📁 FILES & DIRECTORIES:")
    print("   • Required project directories")
    print("   • File upload and processing folders")
    print("   • Backup and utility directories")
    print("")
    ...
    print("")
    print("📋 DATA & CONTENT:")
    print("   • Equipment relationships with dependency management")
    print("   • Initial admin users with security validation")
    print("   • Parts processing with bulk operations and auto-associations")
    print("   • Drawing associations and active lists")
    print("   • Image processing with AI integration")
    print("   • Advanced duplicate prevention across all imports")
    print("")
    print("🔗 SMART ASSOCIATIONS:")
    print("   • Automatic part-image linking")
    print("   • Drawing-part relationships")
    print("   • Content indexing")
    print("")
    print("📝 IMPORTANT NOTES:")
    print("   • For PostgreSQL: Database server must be running")
    print("   • For PostgreSQL: Database will be created EARLY to prevent import errors")
    print("   • For PostgreSQL: You can choose a custom database name")
    print("   • For SQLite: Database file will be created automatically")
    print("   • Existing data will be preserved")
    print("   • Virtual environment is recommended but optional")
    print("   • Audit system provides comprehensive change tracking")
    print("=" * 70)
    ...
    try:
        ...
        if DATABASE_URL.startswith('postgresql'):
            ...
            try:
                ...
                warning_id(f"   ⚠️ Could not test network connectivity: {e}")

        else:
            info_id("🗃️ SQLite setup detected")
...
def main():
    ...
    try:
        ...
        print("\n" + "=" * 50)
        print("📋 DATA IMPORT & CONFIGURATION SCRIPTS")
        print("=" * 50)
        print("The following scripts will populate your database with initial data:")
        print("• Equipment relationships (PostgreSQL-enhanced with dependency management)")
        print("• Initial admin users (with comprehensive validation and security)")
        print("• Parts data import (with bulk operations and automatic associations)")
        print("• Drawing list import (with duplicate prevention)")
        print("• Image folder processing (with AI integration)")
        print("• BOM data import (with advanced Excel manipulation)")
        print("")
        print("⚠️  Note: All scripts use PostgreSQL framework with duplicate prevention")
        print("💡 Each script will check for existing data and warn you accordingly")

        if setup_state.get('audit_system_enabled'):
            print("🔍 Audit System: All data changes will be automatically tracked")
        ...
        if run_scripts in ['y', 'yes']:
            ...
            info_id("⏭️ Skipped setup scripts. You can run them manually later.")

        # Step 9: Associations
        print("\n" + "=" * 50)
        print("🔗 AUTOMATIC ASSOCIATIONS")
        print("=" * 50)
        print("These create smart links between:")
        print("• Parts ↔ Images")
        print("• Drawings ↔ Parts")
        print("")

        run_associations = input(
        ...
        if run_associations in ['y', 'yes']:
            ...
            info_id("⏭️ Skipped association step. You can run them manually later.")

        # Step 10: Log compression
        print("\n🗜️ Would you like to compress old setup logs?")
        ...
        print("\n" + "=" * 70)
        print("🎉 EMTAC SETUP COMPLETED SUCCESSFULLY!")
        print("=" * 70)
        ...
        if setup_state.get('audit_system_enabled'):
            print("🔍 AUDIT SYSTEM:")
            print("   • Comprehensive change tracking enabled")
            print("   • Central audit_log table created")
            print("   • User activity monitoring ready")
            print("   • Compliance and security auditing active")
            print("")

        print("🤖 AI & INTELLIGENCE:")
        print("   • AI model configurations initialized")
        print("   • Smart search capabilities enabled")
        print("   • Embedding models configured")
        print("")
        print("📁 FILES & STRUCTURE:")
        print("   • All required directories created")
        print("   • File processing folders ready")
        ...
        if run_scripts in ['y', 'yes']:
            ...
            print("   • Initial admin users with security validation")
            print("   • Parts processing with bulk operations and auto-associations")
            print("   • Drawing associations with duplicate prevention")
            print("   • Image processing with AI integration")
            print("   • Comprehensive duplicate detection across all imports")
            if setup_state.get('audit_system_enabled'):
                print("   • All imported data automatically audited")
            print("")
        if run_associations in ['y', 'yes']:
            print("🔗 SMART ASSOCIATIONS:")
            print("   • Automatic part-image links created")
            print("   • Drawing-part relationships established")
            print("")
        ...
        try:
            ...
            if setup_state['custom_database_url']:
                db_name = setup_state['database_name']
                print(f"   Database Name: {db_name} (custom)")
            ...
            if db_config.is_postgresql:
                print("   Optimizations: ✅ PostgreSQL features enabled")

            print(f"\n🚀 NEXT STEPS:")
            print("   1. Your EMTAC system is now fully configured")
