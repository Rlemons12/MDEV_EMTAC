# modules/initial_setup/load_bom_loadsheet.py
...
def prompt_for_source_file():
    initializer_logger.info("Prompting for source file.")
    ...
    if os.path.exists(file_path) and os.path.basename(file_path).startswith("bom_for_"):
        initializer_logger.info(f"Source file found: {file_path}")
        return file_path
    else:
        initializer_logger.error(f"File does not exist or does not start with 'bom_for_': {file_path}")
...
def prompt_for_target_file(source_file):
    initializer_logger.info("Generating target file name based on source file.")
    ...
    if not os.path.exists(DB_LOADSHEET_BOMS):
        ...
        initializer_logger.info(f"Directory {DB_LOADSHEET_BOMS} created.")
    ...
    if os.path.exists(file_path):
        initializer_logger.info(f"Target file already exists: {file_path}")
        print(f"The file {file_path} already exists and will be updated.")
    else:
        initializer_logger.info(f"Creating new target file: {file_path}")
...
def copy_bom_sheet_to_target(source_path, target_path):
    initializer_logger.info(f"Copying BOM sheet from {source_path} to {target_path}.")
    ...
    if "BOM" in wb_source.sheetnames:
        ...
        if os.path.exists(target_path):
            ...
            initializer_logger.info(f"Opened existing target workbook: {target_path}")
        else:
            wb_target = Workbook()
            default_sheet = wb_target.active
            wb_target.remove(default_sheet)
            initializer_logger.info(f"Created new target workbook: {target_path}")
        ...
        initializer_logger.info(f"Created new sheet: {new_bom_sheet_name}")
        ...
        initializer_logger.info(f"Copied BOM sheet data to {new_bom_sheet_name} in target workbook.")
        ...
        if "part_position_image" not in wb_target.sheetnames:
            ...
            initializer_logger.info(f"Created new sheet: part_position_image")

        # Save the target workbook
        wb_target.save(target_path)
        initializer_logger.info(f"Saved target workbook: {target_path}")
    else:
        initializer_logger.error(f"The source workbook does not contain a sheet named 'BOM'.")
...
def process_row(part_position_image_sheet, item_number, photo, description, manufacturer_description):
    if photo:
        ...
        initializer_logger.info(f"Added entry: Item Number: {item_number}, Photo: {photo}, Description: {full_description}")

def match_items_and_update_sheet(target_path):
    initializer_logger.info("Matching items and updating part_position_image sheet.")
    wb_target = load_workbook(target_path)
    ...
    initializer_logger.debug(f"BOM sheet headers: {bom_headers}")
    initializer_logger.debug(f"Photo list sheet headers: {photo_list_headers}")
    ...
    with ThreadPoolExecutor() as executor:
        for row in bom_sheet.iter_rows(min_row=2, values_only=True):
            ...
            initializer_logger.debug(f"Processing Item Number: {item_number} (Full row data: {row})")

            if item_number and item_number.startswith("A"):
                item_number = item_number[1:]  # Remove leading 'A' if present
                initializer_logger.debug(f"Stripped leading 'A': {item_number}")
            ...
            for photo_row in photo_list_sheet.iter_rows(min_row=2, values_only=True):
                ...
                if part_number_prefix == photo_part_number_prefix:
                    initializer_logger.info(f"Match found: Item Number: {item_number} - Part Number: {photo_row[0]}")
                    match_count += 1
                    ...
                    if match_count >= 5:
                        initializer_logger.info("Reached the limit of 5 matches. Stopping further processing.")
                        break
    ...
    wb_target.save(target_path)
    initializer_logger.info(f"part_position_image sheet updated in {target_path}.")

def main():
    initializer_logger.info("Starting bom_loadsheet process.")
    ...
    initializer_logger.info("bom_loadsheet process completed.")
