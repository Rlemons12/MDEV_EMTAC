...
from emtacdb_fts import (Area, EquipmentGroup, Model, AssetNumber, Location, SiteLocation, Problem, Solution, Base, Position, ProblemPositionAssociation, 
    split_text_into_chunks, generate_embedding, extract_images_from_pdf, create_position, Document)
...
def add_tsg_loadsheet_to_document_table_db(file_path, area_data, equipment_group_data, model_data, asset_number_data, location_data, problem_data, solution_data):
    try:
        ...
        # Create a new session for the whole operation
        with Session() as session:
            ...
            for index, row in df.iterrows():
                ...
                logger.info(f"Processing row {index + 1} - Problem: {problem_desc}, Location: {location_name}, Model: {model_name}, Solution: {solution_desc}")
                ...
                position_id = create_position(None, None, model_id, None, location_id, None)  # Adjust parameters as needed
                if not position_id:
                ...
                for i, chunk in enumerate(text_chunks):
                    ...
                    if embedding is not None:
                        ...
                        chunk_document = Document(
                            name=f"Imported from Excel - Chunk {i+1}",
                            file_path=os.path.relpath(file_path, DATABASE_DIR),
                            content=padded_chunk,
                            complete_document_id=document_id,
                            embedding=embedding_bytes
                        )
                        session.add(chunk_document)
                        session.commit()
                        logger.info(f"Added chunk {i+1} of document: Imported from Excel")
                    else:
                        logger.error(f"Failed to add chunk {i+1} of document: Imported from Excel")
                        return None, False
...
document_id, success = add_tsg_loadsheet_to_document_table_db(load_sheet_path, area_data, equipment_group_data, model_data, asset_number_data, location_data, problem_data, solution_data)
if success:
